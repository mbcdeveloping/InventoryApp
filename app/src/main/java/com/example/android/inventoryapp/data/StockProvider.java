...
public class StockProvider extends ContentProvider {
    public final static String LOG_TAG = StockProvider.class.getSimpleName();
    /**
     ...
     */
    private static final int STOCK_ITEM_ID = 101;
    private static final UriMatcher sUriMatcher = new UriMatcher(UriMatcher.NO_MATCH);

    // Static initializer. This is run the first time anything is called from this class.
    static {
        sUriMatcher.addURI(StockItemContract.CONTENT_AUTHORITY, StockItemContract.PATH_STOCK, STOCK);
        ...
        sUriMatcher.addURI(StockItemContract.CONTENT_AUTHORITY, StockItemContract.PATH_STOCK + "/#", STOCK_ITEM_ID);
    }
    ...
    public Cursor query(Uri uri, String[] projection, String selection, String[] selectionArgs,
        ...
        switch (match) {
            case STOCK:
                ...
                break;
            case STOCK_ITEM_ID:
                // For the STOCK_ITEM_ID code, extract out the ID from the URI.
                // For an example URI such as "content://com.example.android.inventoryapp/stock/4",
    ...
    private Uri insertItem(Uri uri, ContentValues values) {
        ...
        String title = values.getAsString(StockEntry.PRODUCT_TITLE);
        if (title == null) {
            throw new IllegalArgumentException("Stock Item requires a name");
        }
        // Check that the brand is valid
        String brand = values.getAsString(StockEntry.BRAND);
        if (brand == null) {
            throw new IllegalArgumentException("Stock Item requires a brand");
        }

        // Check that the quantity is valid
        Integer quantity = values.getAsInteger(StockEntry.QUANTITY);
        if (quantity != null && quantity < 0) {
            throw new IllegalArgumentException("The quantity must be greater than 0");
        }

        //Check th price
        Integer price = values.getAsInteger(StockEntry.PRICE);
    ...
    private int updateItem(Uri uri, ContentValues values, String selection, String[] selectionArgs) {
        ...
        if (values.containsKey(StockEntry.PRODUCT_TITLE)) {
            String title = values.getAsString(StockEntry.PRODUCT_TITLE);
            if (title == null) {
        ...
        // check that the brand value is valid.
        if (values.containsKey(StockEntry.BRAND)) {
            String brand = values.getAsString(StockEntry.BRAND);
            if (brand == null) {
        ...
        if (values.containsKey(StockEntry.QUANTITY)) {
            Integer quantity = values.getAsInteger(StockEntry.QUANTITY);
            if (quantity != null && quantity < 0) {
        ...
        if (values.containsKey(StockEntry.PRICE)) {
            String price = values.getAsString(StockEntry.PRICE);
